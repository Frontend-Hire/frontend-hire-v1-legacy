...
<VideoEmbed id='1WwsAMl2MkQ' title='TypeScript: Migrate useState-1 | React Coding | Easy Practice Question | Frontend Hire' />
...
```tsx title="Task.tsx" {1-5, 7} showLineNumbers
type TaskProps = {
  task: any;
  idx: any;
  onDelete: any;
...
```tsx title="AddTask.tsx" {3-5, 7, 10} showLineNumbers
...
![Editor suggesting the type](./type-1.webp)
...
![Editor suggesting the type of parameter e](./type-2.webp)
...
![Task type is different](./type-3.webp)
...
```ts title="App.tsx" showLineNumbers {4,7}
import React from 'react';
...
```ts title="Task.tsx" showLineNumbers {1,4}
import type { Task as TaskType } from './types';
