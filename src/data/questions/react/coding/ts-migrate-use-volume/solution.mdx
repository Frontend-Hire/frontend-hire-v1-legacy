...
Though we now pass the type from the `App` component to the `useVolume` hook. TypeScript still does not know for a certain that a `volume` property is available on the `ref` object. We can fix this by letting it know that the Generic `T` extends `HTMLAudioElement` or `HTMLVideoElement`:
