...
describe("reverseLinkedList", () => {
    ...
    it("should reverse a linked list with two elements", () => {
        const linkedList = new LinkedList();
        linkedList.add(1);
        linkedList.add(2);
        reverseLinkedList(linkedList);
    ...
    it("should return same list if it is empty", () => {
        const linkedList = new LinkedList();
        reverseLinkedList(linkedList);
        expect(linkedList.getList()).toEqual([]);
    });

    it("should return same list if it has only one element", () => {
        const linkedList = new LinkedList();
        linkedList.add(1);
        reverseLinkedList(linkedList);
...
As we have already written the test cases, we can see that the test cases are failing. We need to write the code to pass the test cases.

We will start by creating a linked list.
Don't know how to create a linked list? Check out [this](http://localhost:3000/questions/dsa/coding/linked-list-implementation) article.

As now you know how to create a linked list, let's write the code to reverse a linked list.
...
We have successfully reversed the linked list.

Let's run the test cases to see if our code is working fine or not.

```sh
pnpm vitest index.test.js
```

![Test Cases Passed](./assets/test-cases-passed.webp)
