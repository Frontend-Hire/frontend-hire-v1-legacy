...
But where do we add this entire code? In SvelteKit, we have a `src/routes/+pages.svelte` file which indicates the root URL of our app.

Putting it all together, we should have the following `+pages.svelte` file:

<CustomCodeViewer filename="src/routes/+pages.svelte">
```svelte
<script lang="ts">
  type Priority = 'p1' | 'p2' | 'p3';
