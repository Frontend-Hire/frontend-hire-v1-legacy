...
Why the `_` prefix in the `_components` directory? Though App router allows for [safe colocation by default](https://nextjs.org/docs/app/building-your-application/routing/colocation#safe-colocation-by-default), there is still a risk that someone creates a `page.tsx` inside the `components` directory and it gets picked up by the router. To avoid this, we use the `_` prefix to indicate that this directory should not be picked up by the router and is [a private directory](https://nextjs.org/docs/app/building-your-application/routing/colocation#private-folders).
